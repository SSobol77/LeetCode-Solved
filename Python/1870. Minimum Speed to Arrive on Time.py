...
For example, if the 1^st train ride takes 1.5 hours, you must wait for an additional 0.5 hours before you can depart 
    ...
    def minSpeedOnTime(self, dist: List[int], hour: float) -> int:
        def isPossible(speed):
            ...
            for i in range(len(dist) - 1):
                total_time += -(-dist[i] // speed)  # Equivalent to math.ceil(dist[i] / speed)
            total_time += dist[-1] / speed
            return total_time <= hour

        # Binary search
        left, right = 1, max(dist) * 100
        if isPossible(right):  # Check if it's possible with the maximum speed
            while left < right:
                mid = (left + right) // 2
                if isPossible(mid):
                    right = mid
